...
public class Game extends JPanel implements Runnable {
	...
	private void update() {
		...
		if(keyboard.keyPressedOnce(KeyEvent.VK_2) || Math.random() < 0.015) {
			GameUnit u = new GameUnit(GameUnit.Faction.ENEMY);
			u.placeAt((int)(Math.random() * sWidth), (int)(Math.random() * sHeight));
			units.add(u);
		}		
		if(keyboard.keyPressedOnce(KeyEvent.VK_3) || Math.random() < 0.005) {
			placePickup(Pickup.Type.PLASMID, (int)(Math.random() * sWidth), (int)(Math.random() * sHeight));
		}
		...
		for(GameUnit e: units) {
			if(e == player) {
				((Player) e).update(units, pickups);
			} else {
				e.update(units, player);
			}
			if(!e.alive) {
				dead.add(e);
			}
